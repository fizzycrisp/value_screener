...
def fetch_yfinance(tickers: List[str], timeout: float = 10.0) -> List[FinancialRow]:
    ...
    for t in tickers:
        try:
            tk = yf.Ticker(t)
            info = {}
            # fast_info and info can both be useful
            try:
            ...
            if bs is not None and hasattr(bs, "index"):
                ...
                # If only long-term debt present, still use it
            ...
            if cf is not None and hasattr(cf, "index"):
                ...
                if capex is not None:
                    capex = float(capex)
                    # Yahoo capex is usually negative; make it negative value explicit
                    # We'll subtract capex later, so keep the sign as-is.
            ...
            rows.append(FinancialRow(ticker=t, name=None, price=None, market_cap=None,
